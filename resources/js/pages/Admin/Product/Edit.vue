...
const form = useForm({
    ...
    compare_at_price: props.product.compare_at_price ? parseFloat(props.product.compare_at_price) : null,
    ...
    low_stock_threshold: props.product.low_stock_threshold ? parseFloat(props.product.low_stock_threshold) : null,
    allow_backorder: props.product.allow_backorder ?? false,
    main_image: null,
    gallery_images: [],
    category_id: props.product.category_id || null,
    brand_id: props.product.brand_id || null,
    collection_id: props.product.collection_id || null,
    gender_id: props.product.gender_id || null,
    scent_family_id: props.product.scent_family_id || null,
    tags: props.product.tags ? props.product.tags.map(tag => tag.id) : [],
    labels: props.product.labels ? props.product.labels.map(label => label.id) : [],
    discounts: props.product.discounts ? props.product.discounts.map(discount => discount.id) : [],
    attributes: [],
    ...
    dimension_unit: props.product.dimension_unit || 'cm',
    taxable: props.product.taxable ?? true,
    tax_rate: props.product.tax_rate ? parseFloat(props.product.tax_rate) : null,
    requires_shipping: props.product.requires_shipping ?? true,
    shipping_weight: props.product.shipping_weight ? parseFloat(props.product.shipping_weight) : null,
...
const initializeAttributes = () => {
    if (props.product.attributes && props.product.attributes.length > 0) {
        productAttributes.value = props.product.attributes.map(attr => ({
...
onMounted(() => {
    ...
    if (props.product.main_image_url) {
        mainImagePreview.value = props.product.main_image_url;
    }

    // Initialize gallery images
    if (props.product.gallery_images_urls && props.product.gallery_images_urls.length > 0) {
        existingGalleryImages.value = props.product.gallery_images_urls;
    }
    ...
    if (props.product.meta_title || props.product.meta_description || props.product.meta_keywords) {
...
<template>
    ...
    <AppSidebarLayout :title="`Edit ${product.name}`" :breadcrumbs="breadcrumbItems">
            ...
            <div class="rounded-lg border border-border bg-card p-6">
                <form @submit.prevent="save" class="flex flex-col gap-6">
                    ...
                    <div class="flex justify-end gap-2 mt-4 pt-4 border-t">
                        ...
                        <Button type="submit" label="Update Product" icon="pi pi-check" :loading="form.processing"
