...
const form = useForm({
    ...
    slug: props.flashSale.slug || '',
    description: props.flashSale.description || '',
    start_date: props.flashSale.start_date ? new Date(props.flashSale.start_date) : null,
    end_date: props.flashSale.end_date ? new Date(props.flashSale.end_date) : null,
    banner_image: null,
    discount_type: props.flashSale.discount_type || 'percentage',
    discount_value: props.flashSale.discount_value ? parseFloat(props.flashSale.discount_value) : null,
    max_products: props.flashSale.max_products || null,
    is_active: props.flashSale.is_active ?? true,
    is_featured: props.flashSale.is_featured ?? false,
    sort_order: props.flashSale.sort_order || 0,
    meta_title: props.flashSale.meta_title || '',
    meta_description: props.flashSale.meta_description || '',
    meta_keywords: props.flashSale.meta_keywords || '',
...
const currentBanner = ref(props.flashSale.banner_image);
...
const breadcrumbItems = computed(() => [
    ...
    {
        title: `Edit: ${props.flashSale.name}`,
        href: FlashSaleController.edit.url(props.flashSale.id),
    },
]);
...
const initializeProducts = () => {
    if (props.flashSale.products && props.flashSale.products.length > 0) {
        flashSaleProducts.value = props.flashSale.products.map(product => ({
            product_id: product.id,
            discount_type: product.pivot.discount_type || null,
            discount_value: product.pivot.discount_value ? parseFloat(product.pivot.discount_value) : null,
...
onMounted(() => {
    if (props.flashSale.banner_image_url) {
        bannerPreview.value = props.flashSale.banner_image_url;
    }
    initializeProducts();
    if (props.flashSale.meta_title || props.flashSale.meta_description || props.flashSale.meta_keywords) {
...
const save = () => {
    ...
    })).post(FlashSaleController.update.url(props.flashSale.id), {
        preserveScroll: true,
        forceFormData: true,
    });
};
...
<template>
    ...
    <AppSidebarLayout :title="`Edit: ${flashSale.name}`" :breadcrumbs="breadcrumbItems">
            ...
            <div class="rounded-lg border border-border bg-card p-6">
                <form @submit.prevent="save" class="flex flex-col gap-6">
                    ...
                    <div class="flex justify-end gap-2 mt-4 pt-4 border-t">
                        ...
                        <Button type="submit" label="Update Flash Sale" icon="pi pi-check" :loading="form.processing"
