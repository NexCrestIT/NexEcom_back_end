...
// Watch for type changes - clear values if type changes away from select/multiselect
...
function save() {
    form.post(AttributeController.store.url(), {
        preserveScroll: true,
        forceFormData: false,
        onSuccess: () => {
            form.reset();
...
function cancel() {
    form.get(AttributeController.index.url());
}
</script>
...
<template>
    ...
    <AppSidebarLayout title="Create Attribute" :breadcrumbs="breadcrumbItems">
