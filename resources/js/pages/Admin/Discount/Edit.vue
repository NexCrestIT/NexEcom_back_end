...
const form = useForm({
    ...
    minimum_purchase: props.discount.minimum_purchase || null,
    maximum_discount: props.discount.maximum_discount || null,
    usage_limit_per_user: props.discount.usage_limit_per_user || null,
    total_usage_limit: props.discount.total_usage_limit || null,
    start_date: props.discount.start_date ? new Date(props.discount.start_date) : null,
    end_date: props.discount.end_date ? new Date(props.discount.end_date) : null,
    is_active: props.discount.is_active ?? true,
    is_first_time_only: props.discount.is_first_time_only ?? false,
    free_shipping: props.discount.free_shipping ?? false,
    applicable_categories: props.discount.applicable_categories || [],
    applicable_products: props.discount.applicable_products || [],
    excluded_categories: props.discount.excluded_categories || [],
    excluded_products: props.discount.excluded_products || [],
    sort_order: props.discount.sort_order || 0,
    meta_title: props.discount.meta_title || '',
    meta_description: props.discount.meta_description || '',
    meta_keywords: props.discount.meta_keywords || '',
});
...
if (props.discount.meta_title || props.discount.meta_description || props.discount.meta_keywords) {
...
const breadcrumbItems = computed(() => [
    ...
    {
        title: `Edit: ${props.discount.name}`,
        href: DiscountController.edit.url(props.discount.id),
    },
]);

function save() {
    form.post(DiscountController.update.url(props.discount.id), {
        preserveScroll: true,
    });
}

function cancel() {
    router.visit(DiscountController.index.url());
}
</script>
...
<template>
    ...
    <AppSidebarLayout :title="`Edit: ${discount.name}`" :breadcrumbs="breadcrumbItems">
            ...
            <div class="rounded-lg border border-border bg-card p-6">
                <form @submit.prevent="save" class="flex flex-col gap-6">
                    ...
                    <div class="flex justify-end gap-2 mt-4 pt-4 border-t">
                        ...
                        <Button type="submit" label="Update Discount" icon="pi pi-check" :loading="form.processing"
